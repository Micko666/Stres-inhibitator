...
namespace StressTraining.Pressure
{
    ...
    public sealed class PressureController : MonoBehaviour
    {
        ...
        public void Begin(int pressureSeed, int pressureLevel)
        {
            ...
            _isPaused = false;
            IsRunning = true;
            // The bed starts with the corridor and runs under the whole active part,
            // so the space has room tone from Stable onward instead of dead silence
            // between the scheduled events.
            _audio?.StartAmbience(_timeline.Level.audioRampCap);
        }
        ...
        public void TickActive(float dt, double remainingFraction)
        {
            ...
            ApplyLights();
            _audio?.SetAmbienceIntensity(CurrentIntensity, _timeline.Level.audioRampCap);
            ApplyCracks();
            ApplyShake(dt);
        ...
        /// <summary>Resources path holding the pressure sound effects (see Assets/Resources).</summary>
        private const string PressureAudioResourcePath = "Pressure/Audio/";

        /// <summary>
        /// Loads the optional pressure sound effects. Clips remain OPTIONAL by design —
        /// PressureAudioController tolerates nulls — so a missing folder must never break
        /// a session; it simply leaves the pressure layer silent, as it was before any
        /// audio existed in the project.
        /// </summary>
        private void LoadPressureClips()
        {
            if (_audio == null) return;
            _audio.ConfigureClips(
                Resources.Load<AudioClip>(PressureAudioResourcePath + "Rumble"),
                Resources.Load<AudioClip>(PressureAudioResourcePath + "CeilingCreak"),
                Resources.Load<AudioClip>(PressureAudioResourcePath + "DustBurst"),
                Resources.Load<AudioClip>(PressureAudioResourcePath + "CriticalWarning"),
                Resources.Load<AudioClip>(PressureAudioResourcePath + "CorridorAmbience"));
        }
