...
namespace StressTraining.Session
{
    ...
    public sealed class PlayerFallController : MonoBehaviour
    {
        ...
        {
            if (_armed != armed) Core.FlowTrace.Log("Fall", "SetArmed = " + armed);
            _armed = armed;
        ...
        /// <summary>
        /// Whether standing over a hole may actually cost the participant. Arming
        /// follows the zone, but the demo teaches the tasks and must never drop
        /// anyone through the floor, so it turns the consequence off while a scored
        /// production session turns it back on.
        /// </summary>
        public bool ConsequenceEnabled
        {
            get => _consequenceEnabled;
            set
            {
                if (_consequenceEnabled == value) return;
                _consequenceEnabled = value;
                Core.FlowTrace.Log("Fall", "ConsequenceEnabled = " + value);
            }
        }

        private bool _consequenceEnabled = true;

        private void Update()
        {
            if (_falling) { TickLoss(Time.unscaledDeltaTime); return; }
            if (!_armed || !_consequenceEnabled ||
                _config == null || !_config.fallThroughMissingFloorEnabled) return;
            ...
            if (!_corridor.HasFloorAt(ground))
            {
                Core.FlowTrace.Warn("Fall",
                    "nema poda ispod glave na " + ground.ToString("F2") + " → gubitak");
                BeginLoss();
            }
        }
        ...
        {
            if (_falling) return;
            Core.FlowTrace.Log("Fall", "BeginLoss (armed=" + _armed +
                ", consequenceEnabled=" + _consequenceEnabled + ")");
            _falling = true;
