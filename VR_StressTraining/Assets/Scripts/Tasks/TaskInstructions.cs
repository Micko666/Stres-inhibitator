...
namespace StressTraining.Tasks
{
    ...
    public static class TaskInstructions
    {
        ...
        private static string Flanker(TaskLevelParameters p, TaskLevelParameters previous)
        {
            ...
            // The rows are the one change that can make a participant answer the
            // wrong thing rather than merely answer it slower, so they are stated
            // first and in full — before any timing or mix detail.
            if (p.flankerDistractorRows > 0 && previous.flankerDistractorRows == 0)
                sb.Append("Od sada se prikazuju TRI reda strelica. ")
                  .Append("Odgovor se daje prema centralnoj strelici u srednjem redu. ")
                  .Append("Ostale strelice treba ignorisati — red iznad i red ispod ")
                  .Append("nemaju nikakve veze sa tačnim odgovorom. ");
            else if (p.flankerDistractorRows > 0 &&
                     p.flankerDistractorDensity > previous.flankerDistractorDensity)
                sb.Append("Redovi iznad i ispod su gušći nego ranije, ali i dalje ")
                  .Append("ne znače ništa. Odgovor se i dalje daje prema centralnoj ")
                  .Append("strelici u srednjem redu. ");
