...
namespace StressTraining.Session
{
    ...
    /// <summary>How a finished session leaves the active part.</summary>
    public enum SessionEndPath
    {
        /// <summary>Recovery measurement, then the post-session questionnaires.</summary>
        RecoveryThenQuestionnaires = 0,
        /// <summary>Nothing was scored, so no recovery — questionnaires only.</summary>
        QuestionnairesOnly = 1,
        /// <summary>Voluntary abort: straight to validity + save, out of the headset.</summary>
        StraightToValidity = 2
    }

    /// <summary>
    /// Which closing sequence a finished session takes. Pure so the rule itself is
    /// testable: the sequencing in ProductionSessionFlow needs the full Unity scene,
    /// the DECISION does not.
    /// </summary>
    public static class SessionEndPolicy
    {
        /// <param name="preCorridor">Ended before entering the corridor.</param>
        /// <param name="voluntaryAbort">Participant chose End Session while in the active part.</param>
        public static SessionEndPath PathFor(bool preCorridor, bool voluntaryAbort)
        {
            if (preCorridor) return SessionEndPath.QuestionnairesOnly;
            // A voluntary abort makes the session unusable for adaptation the moment it
            // happens, so recovery and the post-session forms would be collected under
            // discomfort and then discarded. Normal completion and TimeExpired keep them:
            // there the participant went through the active challenge and the recovery
            // measurement is meaningful.
            if (voluntaryAbort) return SessionEndPath.StraightToValidity;
            return SessionEndPath.RecoveryThenQuestionnaires;
        }
    }
    ...
    public sealed class ProductionSessionFlow : IDisposable
    {
        ...
        private void EnterPreSessionSsq()
        {
            ...
            _tablet?.SetHeartRateRecordingActive(true);
            _tablet?.ShowIdle("Mjerenje pulsa teče · Priprema 2 od 2 — kratak upitnik");
            RunQuestionnaires(
                new List<QuestionnaireDefinitionData> { QuestionnaireCatalog.BuildSsq() },
                QuestionnairePhase.PreSession,
                "Priprema 2 od 2 — kratak upitnik",
                () =>
                {
            ...
            string body = readiness.IsReady
                ? "Mjerenje pulsa je povezano i snimanje ostaje aktivno kroz cijelu pripremu, " +
                  ...
                  (_preSessionQuestionnairesCompleted
                      ? "Ponavljamo vođeno disanje uz mjerenje polaznog pulsa."
                      : "Slijedi priprema u dva koraka: vođeno disanje uz mjerenje polaznog pulsa, " +
                        "zatim kratak upitnik, pa trening.")
        ...
        private void BeginBreathingReference()
        {
            ...
            _measuring = true;
            _tablet?.SetHeartRateRecordingActive(true);
            _tablet?.ShowIdle("Mjerenje pulsa teče · Priprema 1 od 2 — disanje i mjerenje pulsa");
            ...
            _progressPanel?.Configure("Priprema 1 od 2 — vođeno disanje i mjerenje pulsa",
                "Prati ritam disanja. Iz ove faze uzimamo tvoj polazni puls za ovu sesiju.",
        ...
        {
            Core.FlowTrace.Log("Production", "EnterCorridor");
            // A scored session is the only place where standing over a hole may
            // actually cost the participant; the demo leaves this off.
        ...
        /// <summary>
        /// User chose End Session from the pause menu (forwarded by the coordinator).
        ///
        /// A VOLUNTARY abort skips recovery and every post-session questionnaire. The
        /// participant who stops — most often because they feel unwell — must not be
        /// held in the headset for another 90 s of measurement plus three forms whose
        /// data cannot be used anyway: the session is already unusable for adaptation
        /// the moment it is aborted (SessionValidityEvaluator rules 4 and 7), so the
        /// measurements would be collected under discomfort and then discarded.
        /// Everything recorded up to the abort is still written and kept.
        /// </summary>
        public void HandleUserTermination(UserTerminationReason reason)
        {
            ...
            if (partial?.Block != null && partial.Block.Trials.Count > 0)
                _blockResults.Add(partial);
            FinishBlocks(CompletionStatus.UserTerminated, reason, skipPostMeasurements: true);
        }
        ...
        /// <param name="preCorridor">Aborted before the corridor: nothing was scored.</param>
        /// <param name="skipPostMeasurements">
        /// Voluntary abort from the active session: skip recovery AND the post-session
        /// questionnaires, and go straight to validity + save. Normal completion and
        /// TimeExpired never set this — there the participant finished the active
        /// challenge and the recovery measurement still carries meaning.
        /// </param>
        private void FinishBlocks(CompletionStatus status, UserTerminationReason reason,
            ...
            bool skipPostMeasurements = false)
        {
            ...
            switch (SessionEndPolicy.PathFor(preCorridor, skipPostMeasurements))
            {
                case SessionEndPath.QuestionnairesOnly:
                    _state.ForceTransition(AppState.Recovery, "pre_corridor_abort_recovery_skip");
                    EnterPostSsq();
                    return;

                case SessionEndPath.StraightToValidity:
                    LogEvent("user_abort_fast_exit",
                        "{\"reason\":\"" + reason + "\",\"skipped\":\"recovery,ssq,tlx,stai\"}");
                    _tablet?.ShowIdle("Sesija je prekinuta. Podaci su sačuvani.");
                    _state.ForceTransition(AppState.Recovery, "user_abort_recovery_skip");
                    _state.RequestTransition(AppState.PostSessionQuestionnaires, "user_abort_post_skip");
                    EnterValidity();
                    return;
        ...
        private Adaptation.AdaptationInput BuildAdaptationInput()
        {
            var input = new Adaptation.AdaptationInput
            {
                ...
                // RecoveryEvaluator returns a default summary (averageBpm = -1) when the
                // recovery phase produced no samples at all, so a non-negative average is
                // the authoritative "recovery was measured" marker.
                recoveryMeasured = (Summary.recovery?.averageBpm ?? -1f) >= 0f,
            ...
            // Session-wide arousal delta. HeartRateService.SessionAverageBpm averages the
            // non-paused accepted samples of the whole session and returns -1 when none
            // were accepted; the delta stays at its -999 "unknown" sentinel unless BOTH
            // that average and the reference value exist.
            float sessionAvgBpm = _heartRate.SessionAverageBpm;
            if (sessionAvgBpm > 0 && input.baselineBpm > 0)
                input.sessionAvgBpmDelta = sessionAvgBpm - input.baselineBpm;
        ...
        {
            Core.FlowTrace.Log("Stage", Stage + " → " + stage);
            Stage = stage;
