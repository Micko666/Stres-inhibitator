...
namespace StressTraining.Session
{
    ...
    public sealed class SessionCoordinator : IDisposable
    {
        ...
        /// <summary>
        /// Built on first use so a participant session never pays for it. Isolated by
        /// construction — see <see cref="DeveloperSandbox"/>.
        /// </summary>
        private DeveloperSandbox _devSandbox;
        ...
        public void Tick(double realDeltaSeconds)
        {
            ...
            // The sandbox owns the frame the way the collapse preview does. It has its
            // own session context, so the gate further down — which requires
            // AppState.ActiveSession and the coordinator's own context — would never
            // let a sandbox task advance a single trial.
            if (_devSandbox != null && _devSandbox.IsActive)
            {
                _heartRate.Update(realDeltaSeconds);
                _devSandbox.Tick(realDeltaSeconds);
                _taskRunner.Tick(realDeltaSeconds);
                return;
            }
        ...
        public UserProfileData SelectProfile(string userId)
        {
            ...
            // This cue was recorded and defined but had no call site at all, so the
            // profile screen was silent even though the take existed.
            Voice?.Play(Audio.VoiceCueId.ProfileSelected);
            ShowModeSelection(profile);
            return profile;
        }
        ...
        private void ShowModeSelection(UserProfileData profile)
        {
            ...
            if (_config.developer.developerModeEnabled)
            {
                ...
                options.Add(("⚙ Developer sandbox — koridor po mjeri",
                    () => OpenDeveloperSandbox(profile)));
        ...
        {
            Core.FlowTrace.Warn("Loss",
                "HandlePlayerLost — povratak u Safe Space (preview=" + _previewActive +
                ", sandbox=" + (_devSandbox != null && _devSandbox.IsActive) + ")");
            Voice?.Play(Audio.VoiceCueId.GameOver);
            if (_previewActive) { StopPressurePreview(); return; }

            // Without this the participant is teleported out while the sandbox still
            // believes it owns the corridor, which reads as "it threw me out".
            if (_devSandbox != null && _devSandbox.IsActive) { _devSandbox.HandleLoss(); return; }
            _zones?.EnterSafeSpace();
            _ui?.SetAnchor(_zones?.SafeSpaceUiAnchor);
        }

        /// <summary>
        /// Opens the developer sandbox. Nothing it does can reach a participant's
        /// profile or the adaptation, so it needs no eligibility or schedule check.
        /// </summary>
        private void OpenDeveloperSandbox(UserProfileData profile)
        {
            if (_devSandbox == null)
            {
                _devSandbox = new DeveloperSandbox(_taskRunner, _zones, _ui, _tablet,
                    _infoPanel, _production, _sessions, _paths, _sessionClock, _pauseController)
                {
                    FallController = FallController,
                    Voice = Voice
                };
                _devSandbox.OnExit = () => ShowModeSelection(_profileService.ActiveProfile ?? profile);
            }
            _devSandbox.Open(profile);
        }
        ...
        public void StartDemo()
        {
            ...
            // The demo exists to teach the tasks. Entering the corridor arms the
            // fall consequence by zone alone, which would drop a participant who is
            // only here to practise, so the consequence is disabled for the whole
            // demo and re-enabled by the production flow.
            if (FallController != null) FallController.ConsequenceEnabled = false;

            Core.FlowTrace.Log("Demo", "sesija kreirana " + sessionId);
        ...
        public void ContinueAfterPractice()
        {
            ...
            _tablet?.ShowInstruction(DemoTaskFactory.DisplayName(CurrentDemoTask) + ": blok 1/1",
                "Zadatak koji se boduje počinje poslije odbrojavanja.");
        ...
        private void OnBlockCompleted(TaskRunnerResult result)
        {
            ...
            // A sandbox block must not walk into the demo's stage machine.
            if (_devSandbox != null && _devSandbox.IsActive)
            {
                _devSandbox.HandleBlockCompleted(result);
                return;
            }
        ...
        private void ShowPracticeReview(TaskRunnerResult result)
        {
            ...
            string body =
                ...
                "\n\nMožeš ponoviti vježbu ili nastaviti na zadatak koji se boduje.";
            _infoPanel?.Configure(DemoTaskFactory.DisplayName(CurrentDemoTask) + ": spremno",
                ...
                "Zadatak koji se boduje još nije počeo.");
            if (_infoPanel != null) _ui?.ShowPanel(_infoPanel);
        ...
        private void ShowDemoOverview(UserProfileData profile)
        {
            ...
            _infoPanel?.Configure("Demo — tri kratka zadatka",
                "Profil: " + profile.username + "\nPosljednja sesija: " + last +
                "\n\nSlijede N-back, Go/No-Go i Flanker. Uz svaki ide uputstvo, kratka vježba i jedan kratak zadatak.\n\nUkupno tri zadatka koja se boduju.",
                new List<(string, Action)>
                {
                    ("Pokreni demo", StartDemo),
                    ("Nazad", ShowProfileSelection)
                },
                "Zadatak ne počinje prije uputstva i vježbe.");
            if (_infoPanel != null) _ui?.ShowPanel(_infoPanel);
            _tablet?.ShowIdle("Izaberi „Pokreni demo“ kad želiš.");
        }
        ...
        {
            Core.FlowTrace.Log("Demo", CurrentDemoStage + " → " + stage);
        ...
        private static string PracticeReminder(TaskType task)
        {
            switch (task)
            {
                ...
                case TaskType.GoNoGo:
                    return "GO — pritisni GO. NO GO — ne pritiskaj ništa. Bilježe se i propušteni GO i pogrešan pritisak na NO GO.";
            ...
            switch (task)
            {
                ...
                case TaskType.GoNoGo:
                    ...
                    body = "Na tabletu se prikazuje velika crna riječ na bijeloj pozadini.\n\n" +
                           ...
                           "Vježba sadrži GO i NO GO primjere i tačno ti pokazuje grešku kod " +
                           "propuštenog GO i pogrešnog pritiska na NO GO.";
