...
if (!existingPanorama)
    ...
    return existing.transform;
            ...
            // Platform — sand disc (visual only). A flat colour read as a dull yellow
            // sheet in the headset: with no surface detail the eye gets no sense of
            // scale or material. The procedural grain below costs one 256x256 texture
            // and gives the disc readable sand structure at arm's length.
            var sand = RuntimeVisualUtil.LitTextured(
                BuildSandTexture(), new Color(0.93f, 0.87f, 0.76f), tiling: 5f,
        ...
        /// <summary>
        /// Procedural sand: fine grain, a slower mottling pass so the surface is not
        /// uniformly busy, and faint wind ripples. Generated once and reused — the
        /// SafeSpace is rebuilt on every bootstrap and a fresh 256x256 each time would
        /// leak texture memory on the headset.
        /// </summary>
        private static Texture2D BuildSandTexture()
        {
            if (_sandTexture != null) return _sandTexture;
            ...
            var warm = new Color(0.88f, 0.79f, 0.62f);   // lit grains
            var cool = new Color(0.66f, 0.58f, 0.45f);   // shaded pits
            ...
            for (int y = 0; y < size; y++)
            {
                ...
                for (int x = 0; x < size; x++)
                {
                    ...
                    // Grain: high-frequency tileable noise.
                    float grain = Mathf.PerlinNoise(u * 96f, v * 96f) * 0.55f +
                                  Mathf.PerlinNoise(u * 190f, v * 190f) * 0.25f;
                    // Mottling: slow variation so the grain does not read as flat static.
                    float mottle = Mathf.PerlinNoise(u * 7f, v * 7f) * 0.30f;
                    // Ripples: shallow wind bands, warped by noise so they are not stripes.
                    float warp = Mathf.PerlinNoise(u * 4f, v * 4f) * 1.6f;
                    float ripple = (Mathf.Sin((v * 22f) + warp) * 0.5f + 0.5f) * 0.16f;

                    float t = Mathf.Clamp01(grain * 0.62f + mottle + ripple * 0.75f);
                    Color c = Color.Lerp(cool, warm, t);
                    pixels[y * size + x] = c;
                }
            }
