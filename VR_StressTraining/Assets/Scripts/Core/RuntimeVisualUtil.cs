...
namespace StressTraining.Core
{
    ...
    public static class RuntimeVisualUtil
    {
        ...
        /// </summary>
        /// <summary>
        /// Lit material carrying a texture. Used for surfaces that must still respond
        /// to scene lighting (the SafeSpace sand), unlike <see cref="UnlitTextured"/>
        /// which is for backdrops that must stay evenly bright.
        /// </summary>
        public static Material LitTextured(Texture texture, Color tint, float tiling = 1f,
            float metallic = 0f, float smoothness = 0.15f)
        {
            var m = new Material(LitShader);
