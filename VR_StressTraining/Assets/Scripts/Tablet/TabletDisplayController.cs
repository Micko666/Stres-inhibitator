...
namespace StressTraining.Tablet
{
    ...
    public sealed class TabletDisplayController : MonoBehaviour
    {
        ...
        // Levels 2–3 add one irrelevant row above and one below. They are separate
        // rects so the relevant row keeps its exact geometry (and the layout tests
        // that assert it) untouched. Distractor chevrons are drawn dimmer than the
        // relevant row: they must crowd the display without ever being mistaken for
        // the row the answer comes from. PROJECT_HEURISTIC.
        private const float DistractorRowOffsetY = 68f;
        private static readonly Color DistractorChevronColor =
            new Color(0.91f, 0.94f, 0.99f, 0.45f);

        private RectTransform _flankerRow;
        private ChevronView[] _chevrons;
        private RectTransform[] _distractorRows;      // [0] above, [1] below
        private ChevronView[][] _distractorChevrons;
        ...
        private void BuildFlankerRow(Transform parent)
        {
            ...
            _distractorRows = new RectTransform[2];
            _distractorChevrons = new ChevronView[2][];
            for (int r = 0; r < 2; r++)
            {
                var row = MakeRect(parent, r == 0 ? "FlankerDistractorTop" : "FlankerDistractorBottom");
                row.anchorMin = row.anchorMax = new Vector2(0.5f, 0.5f);
                row.pivot = new Vector2(0.5f, 0.5f);
                row.sizeDelta = _flankerRow.sizeDelta;
                row.anchoredPosition =
                    new Vector2(0f, r == 0 ? DistractorRowOffsetY : -DistractorRowOffsetY);

                var views = new ChevronView[FlankerCount];
                for (int i = 0; i < FlankerCount; i++)
                {
                    float x = (i - (FlankerCount - 1) * 0.5f) * ChevronSpacing;
                    var root = MakeRect(row, "Chevron" + i);
                    root.anchorMin = root.anchorMax = new Vector2(0.5f, 0.5f);
                    root.pivot = new Vector2(0.5f, 0.5f);
                    root.sizeDelta = new Vector2(ChevronHalfWidth * 2f, ChevronHalfHeight * 2f);
                    root.anchoredPosition = new Vector2(x, 0f);

                    views[i] = new ChevronView
                    {
                        Root = root,
                        ArmUpper = MakeChevronArm(root, "ArmUpper", DistractorChevronColor),
                        ArmLower = MakeChevronArm(root, "ArmLower", DistractorChevronColor)
                    };
                }
                _distractorRows[r] = row;
                _distractorChevrons[r] = views;
                row.gameObject.SetActive(false);
            }
        }

        /// <summary>Exposed for tests: the irrelevant rows, [0] above and [1] below.</summary>
        public RectTransform[] FlankerDistractorRowRects => _distractorRows;

        private static Image MakeChevronArm(Transform parent, string name) =>
            MakeChevronArm(parent, name, ChevronColor);

        private static Image MakeChevronArm(Transform parent, string name, Color color)
        {
        ...
        private void SetFlankerVisible(bool visible)
        {
            ...
            if (!visible && _distractorRows != null)
                foreach (var row in _distractorRows)
                    if (row != null) row.gameObject.SetActive(false);
        }

        /// <summary>
        /// Draws the decoded glyph rows (◄ ► ▬). One row is the classic array; three
        /// rows mean the middle one is the relevant stimulus and the outer two are
        /// irrelevant crowding. Anything else falls back to treating the input as a
        /// single relevant row, so a malformed stimulus can never leave the
        /// participant answering an irrelevant row.
        /// </summary>
        private void RenderFlanker(string glyphRows)
        {
            string[] rows = (glyphRows ?? string.Empty)
                .Split(StressTraining.Tasks.FlankerTaskDefinition.RowSeparator);

            bool crowded = rows.Length == 3;
            string relevant = crowded ? rows[1] : (rows.Length > 0 ? rows[0] : string.Empty);

            DrawRow(_chevrons, relevant);
            for (int r = 0; r < _distractorRows.Length; r++)
            {
                bool show = crowded;
                if (show) DrawRow(_distractorChevrons[r], r == 0 ? rows[0] : rows[2]);
                _distractorRows[r].gameObject.SetActive(show);
            }
            _flankerRow.gameObject.SetActive(true);
        }

        private static void DrawRow(ChevronView[] views, string glyphRow)
        {
            for (int i = 0; i < FlankerCount; i++)
            {
                int dir = 0; // neutral
