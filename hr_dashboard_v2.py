...
# ── hr= value scale ─────────────────────────────────────────────────────────
# Mi Fitness does not always report hr= in plain bpm:
#   2026-07-14  single_heart_rate=[HrItem(..., hr=85)]   -> plain bpm
#   2026-08-23  the SAME field emits hr=425 while the band shows 83 bpm
#               (values 390..580, 100% divisible by 5, i.e. bpm * 5)
# A fixed 30..220 filter therefore discarded ~100% of live samples and the
# bridge looked connected but silent. Instead of hard-coding a divisor (which
# would break the older format and any future change), pick the interpretation
# that makes the most values plausible. Mirrors HrItemParser.java exactly.
MIN_BPM, MAX_BPM = 30, 220
SCALE_DIVISOR = 5
MIN_SAMPLES_TO_LOCK = 3

_hr_scale = None  # None = undetermined, 1 = plain, 5 = scaled


def hr_scale_label():
    if _hr_scale is None:
        return "nepoznata"
    return "bpm x5 (dijeli se sa 5)" if _hr_scale == SCALE_DIVISOR else "bpm"
...
def _detect_scale(raw_values):
    """Returns 1, 5, or None when neither interpretation yields anything usable."""
    plain = sum(1 for v in raw_values if _plausible(v))
    scaled = sum(1 for v in raw_values
                 if v % SCALE_DIVISOR == 0 and _plausible(v // SCALE_DIVISOR))
    if plain == 0 and scaled == 0:
        return None
    return SCALE_DIVISOR if scaled > plain else 1
...
def extract_hr_with_ts(line):
    ...
    if items:
        raws = [int(hr) for _, hr in items]
        line_scale = _detect_scale(raws)
        if line_scale is not None and len(raws) >= MIN_SAMPLES_TO_LOCK:
            if _hr_scale != line_scale:
                _hr_scale = line_scale
                print(f"[HR] Detektovana skala vrijednosti: {hr_scale_label()}")
            _hr_scale = line_scale
        scale = _hr_scale or line_scale or 1
        valid = [(int(t), int(hr) // scale) for t, hr in items
                 if _plausible(int(hr) // scale)]
        if valid:
    ...
    for pat in HR_PATTERNS:
        ...
        if m:
            raw = int(m.group(1))
            scale = _hr_scale or _detect_scale([raw]) or 1
            val = raw // scale
            if _plausible(val):
                return val, 0
    return None, 0
