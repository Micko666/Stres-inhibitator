...
- **ADB host se NE smije zakucavati** — telefon dobija DHCP adresu i ona se mijenja
  (bila `192.168.1.222`, 2026-08-23 je `192.168.1.139`). `find_adb_host()` skenira
  lokalni `/24` na TCP 5555; vrijednosti u `start.bat` i configu su samo fallback.
- Mi Fitness (`com.xiaomi.wearable`) je na **user 0** (provjereno 2026-08-23).
  Raniji zapis da je u work profilu (user 150) je zastario. `READ_LOGS` grant treba za user 0.
- HR podaci dolaze samo kad je HR ekran aktivan na satu (~5 min window)
- Band 9 auth key: drzi se iskljucivo lokalno, van repozitorijuma — nikad u kod ni u dokumentaciju
- UDP šalje i na 127.0.0.1:5005 (za Unity)

### ⚠ Skala `hr=` vrijednosti — Mi Fitness je mijenja između verzija

Isto polje `single_heart_rate=[HrItem(... hr=N)]` ne znači uvijek isto:

| Datum | Log | Stvarni puls |
|---|---|---|
| 2026-07-14 | `hr=85` | 85 bpm |
| 2026-08-23 | `hr=425` | 85 bpm (×5) |

Fiksni filter 30–220 je zbog toga **tiho odbacivao ~100% uzoraka** (13.454 od 13.478)
i relej je izgledao povezano a nije slao ništa.

**Ne rješavati zakucanim `/5`.** Skala se detektuje iz podataka
(`HrItemParser.detectScale`, isto i u `hr_dashboard_v2.py`) po dva pravila koja ne mogu
pogriješiti: vrijednost > 220 ne može biti bpm → skalirana je; vrijednost nedjeljiva sa 5
ne može biti ×5 → obična je. Oba formata rade bez izmjene koda.

### Automatsko pronalaženje odredišta (2026-08-23)

Quest IP se više ne mora kucati. Relej emituje heartbeat pakete na LAN broadcast dok
neko ne odgovori ACK-om, pa zaključa tu adresu. **Unity strana nije mijenjana** — već
šalje ACK na izvorišnu adresu svakog validnog protocol-v1 paketa, heartbeat uključen.
Ako ACK-ovi stanu duže od 15 s u automatskom režimu, traženje kreće ponovo
(Quest restartovan, nova DHCP adresa, druga mreža).

**Ručni unos ostaje kao fallback** — ako je polje adrese popunjeno, ono ima prednost
i automatsko traženje se ne aktivira niti kasnije preuzima kontrolu.
...
3. **Pokreni samo build** preko `BuildPipeline.BuildPlayer(BuildPlayerOptions)`:
   ...
   - **Zvati `BuildPipeline.BuildPlayer` sinhrono, direktno u `Execute()`.** MCP poziv
     će timeoutovati poslije 60 s — to je očekivano i ne znači da je build pao.
     Ispisati `START` i `RESULT` markere pa pratiti `Editor.log`.
...
> **2026-09-12: `EditorApplication.delayCall` se NIJE ispraznio.** Build je ostao na
> `[BuildOnly] QUEUED` bez ijednog `START` markera, uz Editor u praznom hodu
> (`playMode: false`, bez grešaka kompilacije). Isti posao pokrenut **sinhrono**
> (`BuildQuestSync`) krenuo je odmah. Zato je sinhroni poziv sada preporučeni način.
> 2026-07-20 je queue-ovana varijanta radila (`RESULT=Succeeded` za ~4:55), pa
> `delayCall` nije pouzdano mrtav — samo nije pouzdan. Fokus prozora i dalje nije
> dokazani uzrok; ne koristiti ga kao dijagnozu.
>
> **Trajanje varira drastično.** Ako je posljednji Android build star (mjeseci),
> prije Gradle faze ide **hladan import shader-graphova** — stotine `.shadergraph`
> fajlova, više od 20 minuta. `Editor.log` tada pokazuje `Start importing ... .shadergraph`
> i to je dokaz da build napreduje. Ne prekidati i ne zaključivati da je zaglavio;
> provjeriti mtime `Editor.log`-a.
...
Stres-inhibitator-main/          ← root, Claude Code otvara odavde
  ...
  platform-tools/                ← ADB
  (lokalni eksperimentalni folderi se ne cuvaju u repozitorijumu)
  VR_StressTraining/
